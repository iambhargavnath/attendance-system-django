...
def export_attendance(request):
    ...
    if course_id:
        records = records.filter(subject__semester__course_id=course_id)
    if semester_id:
    ...
    if status in [Attendance.PRESENT, Attendance.ABSENT]:
    ...
    for record in records:
        course = record.subject.semester.course
        ...
        taken_by_username = ""
        if record.taken_by:
            taken_by_name = record.taken_by.get_full_name() or record.taken_by.username
            taken_by_username = record.taken_by.username
...
def attendance_report(request):
    ...
    semesters = Semester.objects.select_related("course").all().order_by("course__name", "number")
    subjects = Subject.objects.select_related("semester", "semester__course").all().order_by("code")
    students = Student.objects.select_related("course", "enrollment__semester").all().order_by("roll_number")

    if course_id:
        semesters = semesters.filter(course_id=course_id)
        subjects = subjects.filter(semester__course_id=course_id)
        students = students.filter(course_id=course_id)

    if semester_id:
        subjects = subjects.filter(semester_id=semester_id)
        students = students.filter(enrollment__semester_id=semester_id).distinct()
    ...
    return render(
        ...
        "attendance/report.html",
        {
            ...
            "present_count": records.filter(status=Attendance.PRESENT).count(),
            "absent_count": records.filter(status=Attendance.ABSENT).count(),
        },
    )
